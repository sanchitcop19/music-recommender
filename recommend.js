...
var recommended = require('./recommended.json')
const {me, my_email, partner_email, clientId, clientSecret} = require('./config.js')
const fs = require('fs');
...
spotifyApi.clientCredentialsGrant().then(
  function(data) {
    ...
    spotifyApi.getPlaylistTracks('1reu8sDXrwYmZS7hhdmbrN', {}, function(err, data){
      if (err){
        console.log(err);
        return;
      }
      ...
      me_track = tracks.filter(song => song["added_by"]["id"] == me && !recommended.includes(song["track"]["id"]))[0];
      partner_track = tracks.filter(song => song["added_by"]["id"] != me && !recommended.includes(song["track"]["id"]))[0];

      recommended.push(me_track.track.id);
      recommended.push(partner_track.track.id);
      
      fs.writeFile('recommended.json', JSON.stringify(recommended, null, 2), () => {});
